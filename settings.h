// DLR Aircraft Pneumatic Suspension Settings

// Cylinder Middle Position ----------------------------------------
const long middlePositionA = 150;  //  115 200
const long middlePositionB = 180;  //  115 200
const long middlePositionC = 180;  //  115 200


const long middlePositionRange = 25; // 25 start test 50 / 40 was too big
const long relativeABCdist = 40; // 15

// Cylinder Rise Delay ---------------------------------------------
const long DELAY = 5000; // delay interval

// Poti falure value -----------------------------------------------
const long potiERROR = 50;
const long DELAYPOTI = 60000; // 60 seconds zero reading gives alarm
